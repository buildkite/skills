...
5. **After editing any `SKILL.md`, run `./scripts/build-power.sh` and commit the regenerated `steering/` files.** The Kiro power (root `POWER.md` + `mcp.json` + `steering/`) is derived from the skills; CI fails on drift.
