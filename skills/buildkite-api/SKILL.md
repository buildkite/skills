...
| Teams activation | REST enable-only `POST /organizations/{org.slug}/teams/enable`; GraphQL reads `Organization.isTeamsEnabled` | `write_teams`, `change_teams_enabled` organization-admin permission, organization token scope, and a plan with team permissions when currently disabled | Idempotent: returns `204` when newly enabled or already enabled. First enablement delegates to the existing Teams path, records the audit event, and can seed the Everyone team when no teams exist. There is no matching REST read or disable route. [Teams](https://buildkite.com/docs/apis/rest-api/teams) |
...
|---------|-------------|-----|
| Treating REST as read-only | Organization and hosted-resource writes are missed or replaced with unsuitable GraphQL calls | Select the API by capability and consult the capability map |
...
| Treating REST Teams activation as full lifecycle management | Automation looks for unsupported REST activation-state reads or disable operations | Read `Organization.isTeamsEnabled` with GraphQL, enable idempotently with REST, and use existing team APIs for team resources |
